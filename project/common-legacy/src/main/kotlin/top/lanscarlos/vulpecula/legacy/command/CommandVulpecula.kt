...
import top.lanscarlos.vulpecula.legacy.VulpeculaContext
import top.lanscarlos.vulpecula.legacy.bacikal.action.internal.ActionUnicode
import top.lanscarlos.vulpecula.legacy.internal.*
import top.lanscarlos.vulpecula.legacy.utils.runActions
import top.lanscarlos.vulpecula.legacy.utils.toKetherScript
import top.lanscarlos.vulpecula.volatile.VolatileEntityMetadata
import top.lanscarlos.vulpecula.volatile.VolatilePose
    ...
    val test = subCommand {
        dynamic("content") {
            suggestion<Player> { _, _ ->
                VolatilePose.values().map { it.name }
            }
            execute<Player> { sender, _, content ->
                val pose = VolatilePose.values().first { it.name.equals(content, true) }
                VolatileEntityMetadata.updatePose(sender, sender, pose)
            }
        }
    }
