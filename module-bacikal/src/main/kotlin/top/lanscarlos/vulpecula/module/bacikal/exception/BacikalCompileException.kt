...
class BacikalCompileException(
    ...
    private fun buildLiteralMisspelledErrorMessage(source: String, actions: List<ParsedAction<*>>): String {
        ...
        val errorStartIndex = misspelledAction.properties["BACIKAL_START_INDEX"].let(IntApplicative::convert)
        val errorEndIndex = misspelledAction.properties["BACIKAL_END_INDEX"].let(IntApplicative::convert)
        val startLine = misspelledAction.properties["BACIKAL_START_LINE"].let(IntApplicative::convert)
        val endLine = misspelledAction.properties["BACIKAL_END_LINE"].let(IntApplicative::convert)
